...
if ( ! class_exists( 'BWF_Google_Fonts' ) ) {
	class BWF_Google_Fonts {
		...
		public static function is_system_font( $font_name ) {
			$standard_font = file_exists( dirname( __FILE__ ) . '/standard-fonts.php' ) ? include dirname( __FILE__ ) . '/standard-fonts.php' : null;
			if ( $standard_font ) {
		...
		public function is_woofunnel_block( $block_name ) {
			return ! is_null( $block_name ) && ( strpos( $block_name, 'bwfblocks/' ) === 0 || strpos( $block_name, 'sling-block/' ) === 0 );
		}
